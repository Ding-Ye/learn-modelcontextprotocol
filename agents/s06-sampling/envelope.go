...
// s01 onward. By the time a learner reaches s06 they've written `Message`,
// `ID`, and `stdioFramer` five times. The repetition is the lesson — the
// wire shape stops feeling magical, and you stop reaching for a shared
// `mcp` package that doesn't exist. See plan.md "Shared types catalog".
...
// Standard JSON-RPC 2.0 error codes (schema.ts:175-180). In s06 we lean
// most heavily on InvalidParams (-32602) for malformed sampling requests
// (`maxTokens: 0`, unknown content `type`, etc.).
const (
	ParseError     = -32700
...
// Key returns a stable map key for the pending-outbound table in
// outbound.go. We pick string-formed-raw rather than a typed union
// because s06's request matching is purely "did we send this id?".
func (id ID) Key() string { return string(id.raw) }

// Message is the single envelope shape (request / notification / response /
// error response). See s01 for the long-form commentary; here we use it for
// **both** directions — server-side reads inbound, and server-side writes
// outbound sampling requests.
type Message struct {
...
// Framer abstracts the transport. s06 uses stdio; s08 swaps in HTTP behind
// this same interface.
