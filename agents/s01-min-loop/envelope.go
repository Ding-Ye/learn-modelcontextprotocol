...
// Per the MCP spec (schema/2025-11-25/schema.ts:14-16) every wire message
// carries `"jsonrpc": "2.0"`.
...
// ID is a JSON-RPC request id. Standard JSON-RPC allows numeric, string or
// null; MCP forbids null (see schema.ts:124 RequestId = string | number).
...
// Message is the single envelope shape covering JSON-RPC requests,
// notifications, success responses, and error responses (schema.ts:1-180).
// Field semantics:
//   - Request:      jsonrpc, id, method, params (no result/error)
//   - Notification: jsonrpc, method, params (no id, no result/error)
//   - Result:       jsonrpc, id, result (no method/error)
//   - Error:        jsonrpc, id, error (no method/result)
type Message struct {
...
// null. Go's encoding/json sees `"id": null` on a *ID field and sets the
// pointer to nil silently, so we intercept at the Message level.
func (m *Message) UnmarshalJSON(data []byte) error {
	type rawMessage Message
	// Pull out the raw `id` field so we can distinguish absent from null.
	var probe struct {
...
// Framer abstracts a transport. s01..s07 use a stdioFramer; s08 swaps in HTTP.
...
// Reader/Writer pairs. Per the spec (basic/transports.mdx:1-51) stdio
// messages MUST NOT contain embedded newlines; we enforce that on Write
// so a buggy server can't corrupt the stream.
